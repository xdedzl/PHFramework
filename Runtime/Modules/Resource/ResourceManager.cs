...
namespace PHFramework.Resource
{
    /// <summary>
    /// 资源管理器
    ...
    public partial class ResourceManager : GameModuleBase<ResourceManager>
    {
        ...
        public T Load<T>(string assetName) where T : Object
        {
            ...
            {
                throw new PHFrameworkException("load path is null");
            }
        ...
        public IProgress LoadAllAsync<T>(string path, bool isTopOnly, System.Action<IList<T>> callback) where T : Object
        {
            if (IsResources(path))
            {
                throw new PHFrameworkException("Resource not support LoadAllAsync");
            }
        ...
        private string Path2RealPath(string path)
        {
            if (HasPathMap)
            {
                ...
                {
                    throw new PHFrameworkException($"[Resource] There is no resoure whitch path is {path} or PathMapInfo is obsolete");
                }
            }
            else
            {
