...
namespace PHFramework.Event
{
    ...
    public class MessageManager : Singleton<MessageManager>
    {
        ...
        private void OnPreListenerAdd(string eventKey, Delegate listenerBeingAdded)
        {
            ...
            {
                throw new PHFrameworkException(string.Format("Attempting to add listener with inconsistent signature for event type {0}. Current listeners have type {1} and listener being added has type {2}", eventKey, d.GetType().Name, listenerBeingAdded.GetType().Name));
            }
        }
        ...
        private void OnPreListenerRemove(string eventKey, Delegate listenerBeingRemoved)
        {
            if (m_eventDictionary.ContainsKey(eventKey))
            {
                ...
                if (d == null)
                {
                    throw new PHFrameworkException(string.Format("Attempting to remove listener with for event type \"{0}\" but current listener is null.", eventKey));
                }
                else if (d.GetType() != listenerBeingRemoved.GetType())
                {
                    throw new PHFrameworkException(string.Format("Attempting to remove listener with inconsistent signature for event type {0}. Current listeners have type {1} and listener being removed has type {2}", eventKey, d.GetType().Name, listenerBeingRemoved.GetType().Name));
                }
            }
            else
            {
                throw new PHFrameworkException(string.Format("Attempting to remove listener for type \"{0}\" but Messenger doesn't know about this event type.", eventKey));
            }
        }
        ...
        private PHFrameworkException CreateBroadcastSignatureException(string eventKey)
        {
            return new PHFrameworkException($"Broadcasting message \"{eventKey}\" but listeners have a different signature than the broadcaster.");
        }
